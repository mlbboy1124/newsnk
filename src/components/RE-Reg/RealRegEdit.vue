        ...
        <form @submit.prevent="saveRealreg" style="width:60%">
          <div class="form-row">
            ...
            <div class="form-group col">
              ...
              <input type="date" class="form-control" id="contract_date" v-model="realreg.contract_date" required max="9999-12-31">
            </div>
            <div class="form-group col">
              <label for="settlement_date">잔금일</label>
              <input type="date" class="form-control" id="settlement_date" v-model="realreg.settlement_date" required max="9999-12-31">
            </div>
          </div>
          <div class="form-row">
            <div class="form-group col">
              <label for="sell_price">기준금액</label>
              <input type="text" class="form-control" id="sell_price" :value="formatNumber(realreg.sell_price)" @input="updateSellPrice" required>
            </div>
            <div class="form-group col">
              <label for="public_price">시가표준액</label>
              <input type="text" class="form-control" id="public_price" :value="formatNumber(realreg.public_price)" @input="updatePublicPrice" required>
            </div>
            <div class="form-group col">
    ...
    <div v-for="(buyer, index) in realreg.buyers" :key="index" class="buyer-info">
      ...
      <div class="form-row">
        ...
        <div class="form-group col-1 d-flex align-items-center">
          <button v-if="realreg.buyers.length > 1" type="button" class="btn btn-outline-danger btn-sm" @click="removeBuyer(index)">삭제</button>
        </div>
      </div>
    </div>
  ...
  <div class="seller-group">
    ...
    <div v-for="(seller, index) in realreg.sellers" :key="index" class="seller-info">
      ...
      <div class="form-row">
        ...
        <div class="form-group col-1 d-flex align-items-center">
          <button v-if="realreg.sellers.length > 1" type="button" class="btn btn-outline-danger btn-sm" @click="removeSeller(index)">삭제</button>
        </div>
      </div>
    </div>
    <div class="d-flex justify-content-end">
